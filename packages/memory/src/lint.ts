...
const RESERVED_EXPORT_ROOTS = new Set([".obsidian", "_jingler"])
...
export const unsafeMemoryPathReason = (path: string): string | undefined => {
  ...
  if (RESERVED_EXPORT_ROOTS.has(segments[0]?.toLocaleLowerCase("en-US") ?? "")) {
    return "path uses a reserved export namespace"
  }
  return undefined
}
...
const CREDENTIAL_PATTERNS: ReadonlyArray<CredentialPattern> = [
  ...
  },
  {
    ...
    pattern: /\b(?:authorization|proxy-authorization|cookie|set-cookie)\s*:/i
  },
  {
    label: "assigned credential",
    pattern:
      /(?:api[_-]?key|access[_-]?token|auth[_-]?token|session[_-]?token|client[_-]?secret|private[_-]?key|password|passwd|secret)\s*[:=]\s*["']?(?:bearer\s+)?[^\s,"';]{12,}/i
  }
]
