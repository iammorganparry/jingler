/**
 * `@jingler/plugin-sdk` — the renderer half of a plugin.
 *
 * Import from here in the module your manifest names as `ui`. It is an ES
 * module loaded into Jingler's renderer over the `jingler-plugin://` scheme,
 ...
 *
 * ```ts
 * import { defineManifest, definePlugin, useHost, type TabProps } from "@jingler/plugin-sdk"
 *
 ...
 *
 * 1. **Never hardcode a colour.** Every colour in Jingler is a `--sb-*` custom
 ...
 *    life of the window, so Jingler keys your module's URL by the manifest
...
/**
 * Jingler's own wiring. A plugin never imports these — the host uses them to
 * scope the hooks above to one rendered view.
 *
 * Exported from the public entrypoint rather than a private path because the
 * renderer resolves `@jingler/plugin-sdk` to this exact module, and plugins
...
// `@jingler/plugin-sdk/ui`, for the same reason `/host` is separate: this entry
// has to stay importable from a plain Node script. A plugin generates its
// `jingler.plugin.json` by importing its own manifest module, which imports
