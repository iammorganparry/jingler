...
<div data-assignment data-agent-id="worker-ui" data-cli="codex" data-model="gpt-5.6-terra" data-thinking-enabled="true" data-reasoning-effort="xhigh" data-reason="Complex editor schema work." data-status="running"></div>
...
describe("plan doc HTML round-trip", () => {
  it("re-parses to a valid plan with the same projection", () => {
    ...
    expect(html).toContain('data-thinking-enabled="true"')
    expect(html).toContain('data-reasoning-effort="xhigh"')
