...
export const memoryPrompt = (): string =>
  [
    ...
    "Before finishing non-trivial work, silently reflect on at most three standalone decisions, preferences, gotchas, connections, or hard-won findings that a teammate would want again. Exclude progress narration, questions, temporary status, code-review chatter, facts already documented in the repository, credentials, personal data, and claims corrected later in the turn. A valid reflection may find nothing worth storing.",
    "For each durable candidate, search and read accepted memory first so you can drop duplicates or update the right page. Then call memory_propose with baseRevisionId 'new' for a new page or the accepted revision for an update. Publication is agent-owned and automatic; if the response includes a workflowId, poll memory_workflow_status until it reaches a terminal result. Never claim publication from submission alone.",
