...
export interface FakeGitHubRelay {
  ...
  readonly connectedClientIds: () => ReadonlyArray<string>;
  readonly publish: (
    relaySessionId: string,
    event: GitHubRelayEvent,
  ) => number | null;
  readonly failNextConnections: (count?: number) => void;
...
export const startFakeGitHubRelay = async (
  ...
  const replay = (client: ClientState, after: number): void => {
    for (const stored of events.filter(
      (candidate) =>
        candidate.relaySessionId === client.relaySessionId &&
  ...
  webSockets.on("connection", (socket, request) => {
    ...
    const relaySessionId = request.headers.authorization?.startsWith(
      `Bearer ${grant}:`,
    )
      ? request.headers.authorization.slice(`Bearer ${grant}:`.length)
      : "";
    ...
    const client = { clientId, relaySessionId, socket };
    clients.add(client);
    safeSend(socket, {
      type: "hello",
      cursor,
    ...
    socket.on("message", (raw) => {
      ...
      if (
        ...
        const acknowledged = Math.max(
          acknowledgementsByClient.get(clientId) ?? 0,
          parsed.cursor,
        );
  ...
  const url = await new Promise<string>((resolve, reject) => {
    ...
    server.on(
      ...
      (request: IncomingMessage, socket: Socket, head: Buffer) => {
        ...
        const authorized =
          request.headers.authorization?.startsWith(`Bearer ${grant}:`) ===
          true;
        requests.push({ path: requestUrl.pathname, authorized });
        const fail = failedConnections > 0;
        if (fail) failedConnections -= 1;
        if (requestUrl.pathname !== "/events" || !authorized || fail) {
          socket.write(
            `${fail ? "HTTP/1.1 503 Service Unavailable" : "HTTP/1.1 401 Unauthorized"}\r\nConnection: close\r\n\r\n`,
          );
          socket.destroy();
          return;
        }
  ...
  return {
    ...
    requests,
    acknowledgements,
    connectedClientIds: () => [...clients].map((client) => client.clientId),
    publish: (relaySessionId, event) => {
      if (
        deliveryIds.has(event.deliveryId) ||
        semanticKeys.has(event.semanticKey)
      )
        return null;
      deliveryIds.add(event.deliveryId);
      semanticKeys.add(event.semanticKey);
      const cursor = nextCursor++;
      events.push({ relaySessionId, cursor, event });
      for (const client of clients) {
        if (client.relaySessionId !== relaySessionId) continue;
    ...
    disconnectAll: () => {
      for (const client of [...clients])
        client.socket.close(1012, "Fake relay restart");
    },
    failNextConnections: (count = 1) => {
      failedConnections = Math.max(0, count);
    },
    close: async () => {
