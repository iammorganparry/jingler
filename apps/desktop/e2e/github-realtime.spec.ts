...
test("routes live and replayed GitHub feedback into only the linked visible session", async ({
  launchApp
}) => {
  const launched = await launchApp({
    configured: true,
    withRepo: true,
    githubApp: { connected: true, userLogin: "octocat" },
  ...
  await expect.poll(() => githubRelay.requests.some((request) => request.authorized)).toBe(true)

  const first = "Tighten validation before refreshing credentials."
  githubRelay.publish(feedback("delivery-1", "comment-1", 42, first))
  await openSessionByTitle(window, "Auth flow")
  await expect(window.getByText(first, { exact: false })).toHaveCount(1, { timeout: 20_000 })
  ...
  expect(githubRelay.acknowledgements.some((ack) => ack.cursor === 1)).toBe(false)
  ...
  await expect(restarted.window.getByText(first, { exact: false })).toHaveCount(1)
  await expect.poll(() => githubRelay.acknowledgements.some((ack) => ack.cursor === 1)).toBe(true)
  await expect.poll(acceptedTurnCount).toBe(1)
  const currentWindow = restarted.window

  // The fake relay enforces both GitHub delivery and semantic deduplication.
  expect(githubRelay.publish(feedback("delivery-1", "comment-1", 42, first))).toBeNull()
  ...
  githubRelay.publish(feedback("delivery-2", "comment-2", 43, second))
  await openSessionByTitle(currentWindow, "Refresh token")
  await expect(currentWindow.getByText(second, { exact: false })).toHaveCount(1, { timeout: 20_000 })
  await expect.poll(() => githubRelay.acknowledgements.some((ack) => ack.cursor === 2)).toBe(true)

  await openSessionByTitle(currentWindow, "Auth flow")
  await expect(currentWindow.getByText(second, { exact: false })).toHaveCount(0)

  // Feedback arriving while the agent is paused stays in the existing actor's
  // visible queue; no second/hidden run is spawned.
  await openSessionByTitle(currentWindow, "Refresh token")
  await expect(currentWindow.getByRole("button", { name: /Allow once/ })).toBeVisible({
    timeout: 20_000
  })
  const busy = "Also cover the token rotation edge case."
  githubRelay.publish(feedback("delivery-3", "comment-3", 43, busy))
  await expect(currentWindow.getByText(busy, { exact: false })).toHaveCount(1, { timeout: 20_000 })
  expect(githubRelay.acknowledgements.some((ack) => ack.cursor === 3)).toBe(false)
  await currentWindow.getByRole("button", { name: /Allow once/ }).click()
  await expect.poll(() => githubRelay.acknowledgements.some((ack) => ack.cursor === 3)).toBe(true)
  ...
  const busyTurnCounts = () => {
    ...
    return {
      ...
      visible: messages.filter(
        (message) =>
          message.role === "user" &&
          message.parts?.some((part) => part._tag === "Text" && part.text?.includes(busy))
      ).length
    }
  }
