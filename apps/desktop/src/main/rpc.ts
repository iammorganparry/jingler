...
export const memoryExport = (organizationId: string) =>
  memoryTool(organizationId, "memory_export", {}).pipe(
    Effect.flatMap((value) =>
      decodeMemory(MemoryBackendExport, value, "Memory vault export was invalid")
    ),
    Effect.map((vault) => ({
      filename: `jingler-memory-${organizationId}.zip`,
      mediaType: "application/zip",
      encoding: "base64" as const,
      content: createZipArchive(vault.files).toString("base64")
    }))
  )
...
export const mergeCanonicalOrchestrationCheckpoints = (
  ...
  for (const stage of document.projection.stages) {
    ...
    checkpointByAgent.set(agentId, {
      ...
      state:
        prior?.state ??
        (completedStageIds.length > 0 ? "completed" : "queued"),
      completedStageIds,
...
export const createSessionFromPr = (input: CreateSessionFromPrInput) =>
  Effect.gen(function* () {
    ...
    return yield* SessionStore.createFromPr({ ...input, cli: route.cli }, {
      allowSharedCheckout,
      ...route.options
    })
  })
  ...
  "Agent.approvePlan": ({ sessionId, planId, executionMode, revision }) =>
    Effect.gen(function* () {
      ...
      const document =
        before === null
          ? null
          : yield* canonicalPlanForSession(before, planId)
      if (
        before !== null &&
        document !== null &&
