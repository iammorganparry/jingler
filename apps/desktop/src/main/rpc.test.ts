...
describe("RPC handlers", () => {
  ...
  it("falls back to direct session creation when model discovery crashes", async () => {
    ...
    expect(
      sessionCreationDefaults("codex", null, resolution).options.chatRole,
    ).toBe("direct");
  });
  ...
  it("persists mention delivery, same-thread replies, relays, and stale-target notices", async () => {
    ...
    const result = await Effect.runPromise(
      Effect.gen(function* () {
        ...
        const agentRelay = yield* PlanStore.appendAnnotationMessage(
          worktreePath,
          {
            planId: plan.id,
        ...
        const agentRelayMessage =
          agentRelay.plan.annotations[0]!.messages.at(-1)!;
        const retriedAgentRelay = yield* planDispatchExistingMessageWithRouting(
          ...
          },
          {
            ...
            route: () =>
              Effect.succeed({ status: "delivered" as const, reply: null }),
          },
        );
          ...
          },
          {
            ...
            route: (target) =>
              Effect.sync(() => {
                ...
                const next =
                  target.routingId === relayA.routingId ? relayB : relayA;
                return {
                  status: "delivered" as const,
    ...
    const routedMessages = result.routed.document.plan.annotations[0]!.messages;
    ...
    expect(result.partialCalls).toEqual([
      orchestrator.routingId,
      worker.routingId,
    ]);
    ...
    expect(result.cycleCalls).toEqual([
      "subagent:a",
      "subagent:b",
      "subagent:a",
    ]);
    ...
    expect(
      result.budgeted.deliveries.filter(
  ...
  it("restores worker tabs from durable checkpoints after a main-process restart", () => {
    ...
    const restored = restoredOrchestrationSnapshot("session-1", document, [
      {
        agentId: "worker-auth",
        state: "running",
        completedStageIds: [],
  ...
  it("drains work queued by a mid-run amendment without polling or duplicate execution", async () => {
    ...
    const source = (includeAmendment: boolean): PlanPrd => ({
      ...
      stages: includeAmendment
        ? [
            ...
            stageFixture("02", "agent-02", "src/two.ts"),
          ]
        : [stageFixture("01", "agent-01", "src/one.ts")],
      annotations: [],
    });
    ...
    expect(
      result.document?.plan.stages.map((stage) => stage.executionStatus),
