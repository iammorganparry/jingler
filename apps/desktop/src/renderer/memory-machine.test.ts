...
const api: MemoryApi = {
  ...
  export: vi.fn(async (organizationId: string) => ({ filename: `${organizationId}.zip`, mediaType: "application/zip", encoding: "base64" as const, content: "UEs=" }))
}
...
describe("memoryMachine", () => {
  it("clears every organization-owned record before loading the next organization", async () => {
    ...
    expect(switching.matches("loading")).toBe(true)
  ...
  it("loads exact edge evidence and enters an explicit conflict mode", async () => {
    const actor = await startReady()
    actor.send({ type: "MAP.SELECT_EDGE", edgeId: "edge:org-a" })
    await waitFor(actor, (snapshot) => snapshot.matches("ready"))
