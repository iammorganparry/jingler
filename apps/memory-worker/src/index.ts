import { handleMemoryWorkerRequest } from "./api.js"
  ...
  async scheduled(event: ScheduledControllerLike, env: MemoryWorkerEnv): Promise<void> {
    ...
    await Promise.all(
      scheduledLintOrganizations(env).map(async (organizationId) => {
        ...
        try {
          await env.MEMORY_LINT?.create({
            id: workflowId,
            params: { workflowId, organizationId, asOf }
          })
        } catch {
          // A duplicate scheduled delivery reuses the same durable workflow id.
        }
      })
    )
  }
