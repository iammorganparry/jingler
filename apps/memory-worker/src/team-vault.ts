...
export interface VaultStateStorage {
  ...
  save(expectedVersion: number, next: VaultSnapshot): Promise<boolean>
  replaceSearchProjection(projection: SearchProjection): Promise<void>
...
export class InMemoryVaultState implements VaultStateStorage {
  ...
  async save(expectedVersion: number, next: VaultSnapshot): Promise<boolean> {
    if (this.snapshot.version !== expectedVersion) return false
    this.snapshot = decodeSnapshot(JSON.parse(snapshotJson(next)))
    return true
  }

  async replaceSearchProjection(projection: SearchProjection): Promise<void> {
...
export class TeamVault {
  ...
  private async loadPages(snapshot: VaultSnapshot): Promise<Array<MemoryPage>> {
    return Promise.all(
      [...snapshot.heads]
        .sort((left, right) => compareText(left.pageId, right.pageId))
        .map(async (head) => parseMemoryPage(head.path, await this.objects.readMarkdown(head.markdownKey)))
    )
  }
  ...
  private async persist(
    ...
    if (!(await this.state.save(current.version, next))) {
      throw new MemoryVaultError("storage_conflict", "vault state changed concurrently", 409)
    }
    await this.state.replaceSearchProjection(buildSearchProjection(pages, acceptedLog(next)))
    return next
  }
  ...
  async approveProposalSet(
    ...
    return this.serialized(async () => {
      ...
      const conflicts = setProposals.flatMap((proposal) => {
        ...
        return head === undefined || head.revisionId === proposal.baseRevisionId
          ? []
          : [
              {
                pageId: proposal.pageId,
      ...
      for (const proposal of setProposals) {
        ...
        if (head === undefined || candidate === undefined) {
  ...
  async search(query: string, limit = 20, occurredAt = new Date().toISOString()): Promise<VaultSearchResponse> {
    ...
    await this.state.searchPageIds(query, Math.max(limit * 4, 100))
    const response = searchAcceptedPages(pages, query, limit)
    const metric: RetrievalMetric = {
      id: `retrieval:${snapshot.version + 1}:${snapshot.retrievals.length + 1}`,
      occurredAt,
    ...
    await this.serialized(async () => {
      ...
      await this.persist(
        current,
        { ...current, retrievals: uniqueById([...current.retrievals, metric]) },
        await this.loadPages(current)
      )
    })
    return response
  ...
  async rebuildFromR2(): Promise<RebuildResult> {
    return this.serialized(async () => {
      ...
      const committedPublicationRevisions = new Map(
        publicationRecords.map((publication) => [publication.id, new Set(publication.revisionIds)])
      )
      const revisions = uniqueById(
        (await this.objects.listRevisionRecords()).filter((revision) => {
          if (revision.publicationId === undefined) return true
          return committedPublicationRevisions.get(revision.publicationId)?.has(revision.id) === true
        })
      )
      ...
      const retainedProposals = current.proposals.filter(
        (proposal) =>
      ...
      const retainedProposalSets = current.proposalSets.filter((proposalSet) =>
      ...
      await this.persist(
        current,
        {
          ...
          events,
          retrievals: current.retrievals,
          sessionRetrievals: current.sessionRetrievals
        },
        pages
      )
