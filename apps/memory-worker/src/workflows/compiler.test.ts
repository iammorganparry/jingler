...
class VaultCompilerRepository implements CompilerRepository {
  ...
  async readSource(sourceId: string) {
    return this.vault.readSource(sourceId)
  }

  async listAcceptedPages() {
    const heads = await this.vault.listPages()
    return Promise.all(
      heads.map(async (head) => {
        const accepted = await this.vault.readPage(head.pageId)
  ...
  async readNavigation() {
    return this.vault.navigation()
  }

  createProposalSet(input: Parameters<TeamVault["createProposalSet"]>[0]) {
    return this.vault.createProposalSet(input)
  }

  async approveProposalSet(proposalSetId: string, reviewerId: string, acceptedAt: string) {
    const result = await this.vault.approveProposalSet(proposalSetId, reviewerId, acceptedAt)
...
const seedVault = async (bucket = new InMemoryR2Bucket()): Promise<TeamVault> => {
  const vault = await TeamVault.create("org-compiler", new InMemoryVaultState(), bucket)
  await vault.ingestSource(baseSource, "Base evidence")
  await vault.ingestSource(
    compilerSource,
...
describe("durable memory compiler workflow", () => {
  it("compiles a stored source with the bounded deterministic model", async () => {
    ...
    const proposals = (await vault.snapshot()).proposals
    ...
    expect((await vault.listPages()).every((head) => head.revision === 1)).toBe(true)
  })

  it("creates a new cited page when the vault is empty", async () => {
    const vault = await TeamVault.create(
      "org-compiler",
    ...
    expect(result.status).toBe("pending_review")
    const [proposal] = (await vault.snapshot()).proposals
    expect(proposal).toMatchObject({ baseRevisionId: "new", path: expect.stringMatching(/^learnings\//) })
    const approval = await vault.approveProposalSet(
      result.proposalId,
      "reviewer-new-page",
      "2026-08-01T12:00:00.000Z"
    )
    expect(approval.status).toBe("accepted")
    expect(await vault.listPages()).toEqual([
  ...
  it("creates a cited multi-page pending proposal without moving accepted heads and retries idempotently", async () => {
    ...
    expect((await vault.snapshot()).proposalSets).toHaveLength(1)
    expect((await vault.snapshot()).proposals).toHaveLength(2)
    expect((await vault.listPages()).map((head) => head.revision)).toEqual([1, 1])
    for (const proposal of (await vault.snapshot()).proposals) {
  ...
  it("publishes all intended heads together and regenerates every accepted projection", async () => {
    ...
    const before = await captureVaultDerivedFingerprints(vault, "2026-08-02T00:00:00.000Z")
    const approval = await vault.approveProposalSet(
      result.proposalId,
      "reviewer-1",
      "2026-08-01T12:00:00.000Z"
    ...
    expect(approval.status).toBe("accepted")
    expect((await vault.listPages()).map((head) => head.revision)).toEqual([2, 2])
    expect(await vault.search("reversible rollout", 10, "2026-08-02T00:00:00.000Z")).toMatchObject({
      total: 1,
      results: [{ pageId: "beta", revision: 2 }]
    })
    const navigation = await vault.navigation()
    ...
    const graph = await vault.graph({ limit: 20 }, "2026-08-02T00:00:00.000Z")
    expect(graph.edges.some((edge) => edge.kind === "wikilink" && edge.sourceId === "page:alpha")).toBe(
      true
    )
    const dashboard = await vault.dashboard("2026-08-02T00:00:00.000Z")
    expect(dashboard.growth.revisions).toBe(4)
    expect(dashboard.reviewThroughput).toMatchObject({ proposed: 2, accepted: 2, open: 0 })
    const after = await captureVaultDerivedFingerprints(vault, "2026-08-02T00:00:00.000Z")
    expect(after).not.toEqual(before)
    const reconciled = await reconcileVaultFromR2(vault, "2026-08-02T00:00:00.000Z")
    expect(reconciled.converged).toBe(true)

    const rebuilt = await TeamVault.create("org-compiler", new InMemoryVaultState(), bucket)
    expect(await rebuilt.rebuildFromR2()).toEqual({ pages: 2, revisions: 4, sources: 2 })
    expect((await rebuilt.listPages()).map((head) => head.revision)).toEqual([2, 2])
    const rebuiltSnapshot = await rebuilt.snapshot()
    ...
    expect((await rebuilt.dashboard("2026-08-02T00:00:00.000Z")).retrieval.searches).toBeGreaterThan(0)
  })
  ...
  it("supersedes a stale proposal set without partially publishing unaffected pages", async () => {
    ...
    if (result.status !== "pending_review") throw new Error("expected pending review")
    const acceptedAlpha = await vault.readPage("alpha")
    ...
    expect(
      await vault.approveProposal("competing-alpha", "reviewer", "2026-08-01T11:30:00.000Z")
    ).toMatchObject({ status: "accepted" })

    const stale = await vault.approveProposalSet(
      result.proposalId,
      "reviewer",
      "2026-08-01T12:00:00.000Z"
    )
    expect(stale).toMatchObject({ status: "conflict", conflicts: [{ pageId: "alpha" }] })
    expect((await vault.readPage("alpha")).page.body).toContain("newer alpha decision")
    expect((await vault.readPage("beta")).page.revision).toBe(1)
    expect((await vault.getProposalSet(result.proposalId)).status).toBe("superseded")
  })
  ...
  it("ignores uncommitted publication revisions during R2 reconciliation", async () => {
    ...
    const rebuilt = await TeamVault.create("org-compiler", new InMemoryVaultState(), bucket)
    expect(await rebuilt.rebuildFromR2()).toEqual({ pages: 2, revisions: 2, sources: 2 })
    expect((await rebuilt.readPage("alpha")).page.revision).toBe(1)
  })
  ...
  it("polls and reviews by durable handles across independent stateless API instances", async () => {
    ...
    await vault.ingestSource(baseSource, "Base evidence")
    await vault.ingestSource(compilerSource, "Alpha and beta have a durable workflow.")
    for (const accepted of [page("alpha"), page("beta")]) {
      await vault.ingestAcceptedPage({
        revisionId: `revision-${accepted.id}-1`,
        markdown: serializeMemoryMarkdown(accepted),
  ...
  it("auto-publishes only configured changes that are mechanically equivalent", async () => {
    ...
    expect((await unsafeVault.listPages()).map((head) => head.revision)).toEqual([1, 1])
    ...
    expect((await safeVault.readPage("alpha")).page.revision).toBe(2)
  })
})
