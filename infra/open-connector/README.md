...
that every Jingler agent (`claude` / `codex` / `cursor` / `opencode`) draws its MCP
...
Jingler points each agent at this instance's single `/mcp` endpoint at launch (see
...
- **MCP endpoint (what Jingler uses):** http://localhost:3000/mcp
...
(Cursor is read-only in Settings — Jingler doesn't launch it as a run harness.)
