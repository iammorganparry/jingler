...
- **All three agent adapters** (claude, codex, opencode) mapped `spec.cwd || undefined` to *no* cwd, so a session with a missing worktree would have run the agent against Jingler's own source. They now call `requireWorktree`, which throws rather than inheriting: a session with no worktree has nothing legitimate to run.
