...
- **⌘K and ⌘P open it; the sidebar filter moves to ⌘F.** ⌘K is the chord people arrive already knowing, and until now it put the caret in a list filter — while `empty-conversation.tsx` had been rendering a `⌘K Command palette` hint for a palette that did not exist. The filter keeps a chord rather than losing one.
...
- **`contributes.commands` finally has a surface.** `CommandContribution` in `@starbase/core` is documented verbatim as "an entry in the command palette" and `Plugins.invoke` has existed alongside it; there was simply no palette. Commands are read from plugins that **loaded**, not merely from manifests that declared them — a plugin whose module failed to import has no handler behind its entry.
- **Two capabilities crossed the shell boundary to get here.** The terminal dock's visibility lives in the renderer (`onToggleTerminal` / `terminalActive`), and a pane's active tab is pane-local state, now reachable through a `selectTabRequest` nonce handed to the **focused** pane only — broadcasting it would switch all four tabs in a four-way split.
